...
async def start_in_bot(c, msg):  # الشاشه الرئيسيه في البوت
    ...
    if points >= 100:
        if (points >= getvp(bot.me.id, sudo_info.id) or
            ...
            await c.send_log(f'⌯ جاري تحويل {points - 1} نقطه اليك')
            try:
...
async def a_re_send(c: userbot, msg):
    ...
    await c.send_log(f'ميقدرش يحولك نقاط غير بعد {s[0]} ساعه و {s[1]} دقيقه لانه لسه جديد في البوت')


async def main():
    await userbot.start()
    await userbot.join_chat("zgzzz")
    await userbot.join_chat("KTTTTGiFT")
    await userbot.join_chat("KTTTT")
